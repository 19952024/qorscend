...
class PaymentService {
  ...
  initializeProviders() {
    ...
    if (process.env.PAYPAL_CLIENT_ID && process.env.PAYPAL_CLIENT_SECRET) {
      try {
        // New PayPal SDK initialization
        // Note: The new @paypal/paypal-server-sdk may have a different API structure
        // This initialization may need adjustment based on the actual SDK documentation
        const environment = process.env.NODE_ENV === 'production' ? 'live' : 'sandbox';
        this.paypal = {
          clientId: process.env.PAYPAL_CLIENT_ID,
          clientSecret: process.env.PAYPAL_CLIENT_SECRET,
          environment: environment,
          // Store SDK reference for method calls
          sdk: paypal
        };
        logger.info('PayPal payment service initialized');
      } catch (error) {
        logger.warn('PayPal initialization error:', error);
        this.paypal = null;
      }
    } else {
  ...
  async createPayPalOrder(amount, currency = 'USD', description = 'QORSCEND Subscription') {
    ...
    try {
      // Note: The new @paypal/paypal-server-sdk API may differ from the old SDK
      // This implementation may need adjustment based on the actual SDK documentation
      // For now, using a structure that should work with modern REST SDKs
      const orderRequest = {
        intent: 'CAPTURE',
        application_context: {
      ...
      // Attempt to use new SDK structure - may need adjustment
      // If the SDK exports OrdersApi or similar, use that
      if (this.paypal.sdk && this.paypal.sdk.OrdersApi) {
        const ordersApi = new this.paypal.sdk.OrdersApi({
          environment: this.paypal.environment,
          clientId: this.paypal.clientId,
          clientSecret: this.paypal.clientSecret
        });
        const order = await ordersApi.ordersCreate(orderRequest);
        return order;
      } else {
        // Fallback: log warning and return mock structure
        logger.warn('PayPal SDK structure not recognized - please update createPayPalOrder method');
        throw new Error('PayPal SDK API structure needs to be updated');
      }
    } catch (error) {
  ...
  async createPayPalSubscription(planId, startTime) {
    ...
    try {
      ...
      // Attempt to use new SDK structure - may need adjustment
      // If the SDK exports SubscriptionsApi or similar, use that
      if (this.paypal.sdk && this.paypal.sdk.SubscriptionsApi) {
        const subscriptionsApi = new this.paypal.sdk.SubscriptionsApi({
          environment: this.paypal.environment,
          clientId: this.paypal.clientId,
          clientSecret: this.paypal.clientSecret
        });
        const subscription = await subscriptionsApi.subscriptionsCreate(subscriptionRequest);
        return subscription;
      } else {
        // Fallback: log warning and throw error
        logger.warn('PayPal SDK structure not recognized - please update createPayPalSubscription method');
        throw new Error('PayPal SDK API structure needs to be updated');
      }
    } catch (error) {
